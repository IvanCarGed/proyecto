...
<head>
    ...
    <link rel="stylesheet" href="<?php echo esc_url( AGNI_FRAMEWORK_CSS_URL . '/fortun.css' ); ?>" type="text/css" media="all" />   
    <link rel="stylesheet" href="<?php echo esc_url( AGNI_FRAMEWORK_CSS_URL . '/linea-basic.min.css' ); ?>" type="text/css" media="all" />  
    <link rel="stylesheet" href="<?php echo esc_url( get_stylesheet_uri() ); ?>" type="text/css" media="all" />  
</head>
<?php if( $fortun_options['maintenance-mode-choice'] == '2' ){ ?>
<body id="error-page" class="error-page" style="background: <?php echo esc_html( $fortun_options['maintenance-mode-custom-bg']['background-color'] ); ?> url('<?php echo esc_url( $fortun_options['maintenance-mode-custom-bg']['background-image'] ); ?>') <?php echo esc_html( $fortun_options['maintenance-mode-custom-bg']['background-repeat'] ); ?> <?php echo esc_html( $fortun_options['maintenance-mode-custom-bg']['background-attachment'] ); ?>; background-position: <?php echo esc_html( $fortun_options['maintenance-mode-custom-bg']['background-position'] ); ?>; background-size: <?php echo esc_html( $fortun_options['maintenance-mode-custom-bg']['background-size'] ); ?>; ">
<?php } 
else{ ?>
...
<?php endif; // ! did_action( 'admin_head' ) ?>
    <?php 
    if( $fortun_options['maintenance-mode-choice'] == '1' ){
        ...
        <div id="content" class="agni-maintenance-mode-content">
            ...
            <h2>'.esc_html__( 'We\'ll be right back!', 'fortun' ).'</h2>
            <p>'.esc_html__( 'Sorry for the inconvenience. We\'re busy on making something cool for you.', 'fortun' ).'<br/>'.esc_html__( 'Please try after sometime.', 'fortun' ).'</p>
        </div>';
    }
    else{
        $message = $fortun_options['maintenance-mode-custom'];
    }
    echo $message; ?>
</body>
</html>
