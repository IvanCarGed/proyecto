...
class CMB2_REST_Controller_Fields extends CMB2_REST_Controller_Boxes {
	...
	public function update_item( $request ) {
		...
		if ( ! $this->request['value'] ) {
			return new WP_Error( 'cmb2_rest_update_field_error', esc_html__( 'CMB2 Field value cannot be updated without the value parameter specified.', 'cmb2' ), array( 'status' => 400 ) );
		}
	...
	public function modify_field_value( $activity) {
		...
		if ( ! $this->request['object_id'] || ! $this->request['object_type'] ) {
			return new WP_Error( 'cmb2_rest_modify_field_value_error', esc_html__( 'CMB2 Field value cannot be modified without the object_id and object_type parameters specified.', 'cmb2' ), array( 'status' => 400 ) );
		}
		...
		if ( ! $this->field ) {
			return new WP_Error( 'cmb2_rest_no_field_by_id_error', esc_html__( 'No field found by that id.', 'cmb2' ), array( 'status' => 403 ) );
		}
	...
	protected function prepare_field_data( CMB2_Field $field ) {
		...
		foreach ( $field_args as $key => $value ) {
			...
			if ( empty( $value ) || is_scalar( $value ) || is_array( $value ) ) {
				...
				$field_data[ $key ] = sprintf( wp_kses( __( 'Value Error for %s', 'cmb2' ) ), $key );
			}
		}
