...
class CMB2_Types {
	...
	public function render_repeatable_field() {
		...
		<p class="cmb-add-row">
			<button type="button" data-selector="<?php echo esc_attr( $table_id ); ?>" class="cmb-add-row-button button"><?php echo esc_html( $this->_text( 'add_row_text', esc_html__( 'Add Row', 'cmb2' ) ) ); ?></button>
		</p>
	...
	protected function repeat_row( $disable_remover = false, $class = 'cmb-repeat-row' ) {
		...
		<div class="cmb-row <?php echo esc_attr( $class ); ?>">
			...
			<div class="cmb-td cmb-remove-row">
				<button type="button" class="button cmb-remove-row-button<?php echo esc_attr( $disabled ); ?>"><?php echo esc_html( $this->_text( 'remove_row_text', esc_html__( 'Remove', 'cmb2' ) ) ); ?></button>
			</div>
		</div>
